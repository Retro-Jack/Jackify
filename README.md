...
2. **Convert** — All videos in `STAGING_DIR` are converted using HandBrakeCLI with the selected preset. A progress bar is shown for each job; already-converted files are skipped.
   ...
   - **Burned subtitles** — if a matching `.srt` is present (whether extracted or pre-existing), a second HandBrake pass produces a separate `[Burned Subs]` copy of the video with the subtitles rendered into the picture.
